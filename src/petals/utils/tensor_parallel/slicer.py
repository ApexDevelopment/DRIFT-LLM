...
removed: Petals only shards ``nn.Linear`` / ``nn.Embedding`` layers across a few local devices.
"""
...
from petals.utils.tensor_parallel import cross_device_ops
...
class _TensorParallelWrapper(nn.Module):
	...
	def __init__(
		...
		*,
		...
		super().__init__()
		# note: attribute is named `module` on purpose -- petals.utils.misc.get_num_attention_heads
