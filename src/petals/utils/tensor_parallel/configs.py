"""
Per-architecture tensor-parallel slicing configs for the transformer blocks Petals serves.
...
across devices, matching the head-split KV cache in ``petals/server/backend.py``.

Petals manages the KV cache at the *block* level (each ``Wrapped*Block.forward`` owns a
...
from petals.utils.tensor_parallel.communications import CollectiveOperation
from petals.utils.tensor_parallel.slicer import Config
from petals.utils.tensor_parallel.wrapper import PerDeviceTensors
...
def split_falcon_attention_mask(mask, *, rank: int, world_size: int):
    ...
    Petals folds alibi into the 4D mask, giving ``[batch, num_heads, q, kv]`` (per head) for the alibi
