"""
Shared Petals wrapper for Gemma 2 / Gemma 3 decoder layers.

Gemma differs from the plain GQA models in two position-dependent ways that Petals' otherwise
...
normalized to 0 because Petals owns a single-layer KV cache. Softcapping, query pre-attention
...
class WrappedGemmaBlock(BloomLayoutCacheMixin):
    """Mixin bridging Petals' BLOOM-layout cache around a stock Gemma decoder layer.
