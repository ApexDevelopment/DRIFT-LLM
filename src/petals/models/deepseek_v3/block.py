...
class WrappedDeepseekV3Block(BloomLayoutCacheMixin, DeepseekV3DecoderLayer):
    """A Petals wrapper around a stock transformers ``DeepseekV3DecoderLayer``.
    ...
    Two DeepSeek-specific points, both handled here:
      ...
      * whether a layer's FFN is dense or MoE is chosen by the global layer index
        ...
        *true* index, then the cache index is normalized to 0 (Petals owns a single-layer cache), and
      * MLA decompresses the KV latent before caching (HF does this in attention), so Petals keeps
