"""
Shared Petals wrapper for standard dense (MHA/GQA) transformer decoder layers.
...
signature), so a single mixin bridges Petals' BLOOM-style KV layout to/from that contract for
...
class BloomLayoutCacheMixin:
    """Converts Petals' BLOOM-layout ``layer_past`` to/from the ``transformers`` Cache layout.

    Petals stores keys as ``[batch * kv_heads, head_dim, seq]`` and values as
...
class WrappedGQABlock(BloomLayoutCacheMixin):
    """Mixin implementing Petals' cache bridging around a stock dense decoder layer.
