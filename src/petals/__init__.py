...
if not os.getenv("PETALS_IGNORE_DEPENDENCY_VERSION"):
    assert (
        version.parse("5.12.0") <= version.parse(transformers.__version__) < version.parse("6.0.0")
    ), "Please install a proper transformers version: pip install transformers>=5.12,<6.0"
