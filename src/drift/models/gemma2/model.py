...
from drift.models.gemma2.config import DistributedGemma2Config

logger = get_logger(__name__)


class DistributedGemma2Model(FromPretrainedMixin, PTuneMixin, Gemma2Model):
    """Gemma2Model, but all transformer layers are hosted by the swarm"""

    _keys_to_ignore_on_load_missing = PTuneMixin._keys_to_ignore_on_load_missing
    _keys_to_ignore_on_load_unexpected = [r"^model\.layers\."]

    config_class = DistributedGemma2Config

    def __init__(self, config: DistributedGemma2Config, *, dht: Optional[hivemind.DHT] = None):
...
class DistributedGemma2ForCausalLM(FromPretrainedMixin, RemoteGenerationMixin, Gemma2ForCausalLM):
    _keys_to_ignore_on_load_missing = DistributedGemma2Model._keys_to_ignore_on_load_missing
    _keys_to_ignore_on_load_unexpected = DistributedGemma2Model._keys_to_ignore_on_load_unexpected

    config_class = DistributedGemma2Config

    def __init__(self, config: DistributedGemma2Config):
        Gemma2PreTrainedModel.__init__(self, config)
        self.model = DistributedGemma2Model(config)
    ...
    @property
    def transformer(self) -> DistributedGemma2Model:  # For compatibility with RemoteGenerationMixin
        return self.model


class DistributedGemma2ForSequenceClassification(FromPretrainedMixin, Gemma2ForSequenceClassification):
    _keys_to_ignore_on_load_missing = DistributedGemma2Model._keys_to_ignore_on_load_missing
    _keys_to_ignore_on_load_unexpected = DistributedGemma2Model._keys_to_ignore_on_load_unexpected
