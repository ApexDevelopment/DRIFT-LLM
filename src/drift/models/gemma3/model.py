...
from drift.models.gemma3.config import DistributedGemma3Config

logger = get_logger(__name__)


class DistributedGemma3Model(FromPretrainedMixin, PTuneMixin, Gemma3TextModel):
    """Gemma3Model, but all transformer layers are hosted by the swarm"""
...
class DistributedGemma3ForCausalLM(FromPretrainedMixin, RemoteGenerationMixin, Gemma3ForCausalLM):
    ...
    @property
    def transformer(self) -> DistributedGemma3Model:  # For compatibility with RemoteGenerationMixin
        return self.model


class DistributedGemma3ForSequenceClassification(FromPretrainedMixin, Gemma3TextForSequenceClassification):
    _keys_to_ignore_on_load_missing = DistributedGemma3Model._keys_to_ignore_on_load_missing
    _keys_to_ignore_on_load_unexpected = DistributedGemma3Model._keys_to_ignore_on_load_unexpected
