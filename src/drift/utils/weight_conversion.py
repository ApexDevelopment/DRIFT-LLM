...
down_proj}``). transformers does this inside its own loading pipeline, which DRIFT-LLM bypasses
...
architecture transformers can convert, DRIFT-LLM can too — without hard-coding per-model fusions.
"""
import re
...
def _apply_converter(sd: Dict[str, torch.Tensor], converter) -> Dict[str, torch.Tensor]:
    ...
    handled independently. For DRIFT-LLM' single block there is exactly one group.
    """
