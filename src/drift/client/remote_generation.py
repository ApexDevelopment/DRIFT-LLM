...
from drift.client.inference_session import InferenceSession
from drift.client.remote_sequential import RemoteSequential
from drift.utils.misc import DUMMY, docstring_from
...
class RemoteGenerationMixin(_SkipTokensMixin):
        ...
        with context_manager as session:
            ...
            if n_prev_tokens > 0:
                if kwargs.get("num_beams", 1) > 1:
                    logger.warning(
                        "Beam search will not work properly in the resumed drift.InferenceSession "
    ...
    def _fix_generate_kwargs(kwargs: dict):
        ...
        # Support do_sample = {0, 1} for backward compatibility with DRIFT-LLM < 2.1.0
