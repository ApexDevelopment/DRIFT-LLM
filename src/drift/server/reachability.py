...
from drift.constants import REACHABILITY_API_URL
...
def validate_reachability(peer_id, wait_time: float = 7 * 60, retry_delay: float = 15) -> None:
    ...
    for attempt_no in range(math.floor(wait_time / retry_delay) + 1):
        try:
            ...
            if response["success"]:
                logger.info("Server is reachable from the Internet. It will appear at https://health.drift.dev soon")
                return
            ...
            logger.warning(f"Skipping reachability check because health.drift.dev is down: {repr(e)}")
            return
    ...
    raise RuntimeError(
        ...
        f"    1. Choose a specific port for the DRIFT-LLM server, for example, 31337.\n"
        ...
        f"        python -m drift.cli.run_server ... --public_ip {response['your_ip']} --port 31337\n"
